...
DESCRIPTION="NVIDIA Accelerated Graphics Vulkan Beta Driver"
HOMEPAGE="
...
SRC_URI="
	${NV_DEV_URI}vulkan-beta-${PV//./}-linux -> NVIDIA-Linux-x86_64-${PV}.run
	${NV_URI}nvidia-installer/nvidia-installer-${NVSET_PV}.tar.bz2
	${NV_URI}nvidia-modprobe/nvidia-modprobe-${NVSET_PV}.tar.bz2
	${NV_URI}nvidia-persistenced/nvidia-persistenced-${NVSET_PV}.tar.bz2
	${NV_URI}nvidia-settings/nvidia-settings-${NVSET_PV}.tar.bz2
	${NV_URI}nvidia-xconfig/nvidia-xconfig-${NVSET_PV}.tar.bz2
"
S="${WORKDIR}"
...
src_prepare() {
	...
	rm nvidia-modprobe && mv nvidia-modprobe{-${NVSET_PV},} || die
	rm nvidia-persistenced && mv nvidia-persistenced{-${NVSET_PV},} || die
	rm nvidia-settings && mv nvidia-settings{-${NVSET_PV},} || die
	rm nvidia-xconfig && mv nvidia-xconfig{-${NVSET_PV},} || die
