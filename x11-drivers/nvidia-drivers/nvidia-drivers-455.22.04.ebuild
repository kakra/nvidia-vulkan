...
src_compile() {
	...
	if use tools; then
		emake -C "${S}"/nvidia-settings-${NVSET_PV}/src/libXNVCtrl \
			DO_STRIP= \
			LIBDIR="$(get_libdir)" \
			NVLD="$(tc-getLD)" \
			NV_VERBOSE=1 \
		...
		emake -C "${S}"/nvidia-settings-${NVSET_PV}/src \
			DO_STRIP= \
...
src_install() {
	...
	if use tools; then
		...
		if use static-libs; then
			dolib.a "${S}"/nvidia-settings-${NVSET_PV}/src/libXNVCtrl/libXNVCtrl.a

			insinto /usr/include/NVCtrl
			doins "${S}"/nvidia-settings-${NVSET_PV}/src/libXNVCtrl/*.h
		fi

		insinto /usr/share/nvidia/
		newins \
			nvidia-application-profiles-${PV}-key-documentation nvidia-application-profiles-${NVSET_PV}-key-documentation
